...
(* Example Halve_fix_step_example : exists t,
...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  (* fix *)
  (* | <{ fix (\f:T1, \y:T2, t1) }> =>
      if String.eqb x f then t
      else if String.eqb x y then t
      else <{ fix (\f:T1, \y:T2, [x:=s] t1) }>
  end *)
  | <{ fix t1 }> =>
    <{ fix ([x:=s] t1) }>
  end
...
Proof with eauto.
  ...
  - (* tm_let *)
    rename s into y.
    destruct (eqb_spec x y); subst; eapply T_Let...
    + (* x=y *)
