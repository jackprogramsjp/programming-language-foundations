...
(** **** Exercise: 3 stars, advanced (progress_from_term_ind)
    ...
    (* TODO: I SERIOUSLY DON'T GET THIS *)
Theorem progress' : forall t T,
  <{ empty |-- t \in T }> ->
  value t \/ exists t', t --> t'.
Proof with eauto.
  ...
  - inversion Ht; subst.
    destruct (IHt1 _ H2) as [Hv1 | [t1' Hstep1]];
    destruct (IHt2 _ H4) as [Hv2 | [t2' Hstep2]];
    right.

    + eapply canonical_forms_fun in H2; [| exact Hv1].
      destruct H2 as [x0 [t0 Heq]].
      subst.
      exists (<{ [x0 := t2] t0 }>)...
  ...
  - inversion Ht; subst.
    destruct (IHt1 _ H3) as [Hv1 | [t1' Hstep1]].

    + destruct (canonical_forms_bool t1 H3 Hv1)
        as [Heq | Heq];
      subst;
      eauto.
...
Proof.
  ...
  - rewrite H3 in H7.
    inversion H7.
    auto.
  - pose proof (IHe1 _ _ _ H4 H10) as He1.
    inversion He1; auto.
  - pose proof (IHe (s |-> t; Gamma) _ _ H6 H12) as He.
    subst.
    auto.
  - pose proof (IHe2 _ _ _ H7 H15) as He2.
    assumption.
Qed.
(** [] *)
...
Proof.
  ...
  destruct H7 as [T' H7].
  rewrite update_neq in H7; try assumption.
  exists T'.
  assumption.
Qed.
(** [] *)
...
Proof.
  intros t T H x.
  intros Ha.
  pose proof (free_in_context x t T empty Ha H) as Hfree.
  destruct Hfree as [T' Hlookup].
  inversion Hlookup.
Qed.
(** [] *)
...
Proof.
  ...
  - apply T_Var.
    rewrite <- H.
    assert (Hfree : appears_free_in x0 x0).
    {
      apply afi_var.
    }
    specialize (H0 x0 Hfree).
    symmetry.
    assumption.
  - apply T_Abs.
    apply IHhas_type.
...
Theorem progress_for_stlc : forall t T,
  <{ empty |-- t \in T }> ->
  value t \/ exists t', t --> t'.
Proof.
  Admitted.

Theorem preservation_for_stlc : forall t t' T,
  <{ empty |-- t \in T }> ->
  t --> t' ->
  <{ empty |-- t' \in T }>.
Proof.
...
(** **** Exercise: 2 stars, standard (stlc_variation2)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
(λx:Bool. x) true --> true     // normal beta reduction
(λx:Bool. x) true --> zap      // ST_Zap
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_If0_Nonzero : forall (n:nat) t2 t3,
...
Lemma weakening : forall Gamma Gamma' t T,
     ...
     <{ Gamma' |-- t \in T }>.
Proof.
  intros Gamma Gamma' t T Hi Hg.
  generalize dependent Gamma'.
  induction Hg; intros Gamma' Hi;
