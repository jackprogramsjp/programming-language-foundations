...
Proof with eauto.
  ...
  - (* case *)
    ...
    fully_invert_typecheck Gamma t0 T0 TL TR.

    remember (y1 |-> TL ; Gamma) as Gamma1.
    invert_typecheck Gamma1 t1 T1.

    remember (y2 |-> TR ; Gamma) as Gamma2.
  ...
  (* unit *)
  - inversion H0; subst.
    constructor. (* Can also just do eauto here! *)
  (* pairs *)
  - (* pair *)
    invert_typecheck Gamma t1 T1.
    invert_typecheck Gamma t2 T2.
  ...
  - (* fst *)
    rename t into t1.
    fully_invert_typecheck Gamma t1 T1 TL TR.
  ...
  - (* let *)
    rename s into x.
    invert_typecheck Gamma t1 T1.
    remember (x |-> T1 ; Gamma) as Gamma'2.
...
Fixpoint valuef (t : tm) : bool :=
  ...
  | tm_case _ _ _ _ _ => false
  
  (* lists *)
  | <{ nil T }> => true
  | <{ t1 :: t2 }> => andb (valuef t1) (valuef t2)
  | tm_lcase _ _ _ _ _ => false
...
Fixpoint stepf (t : tm) : option tm :=
  ...
  | tm_case t0 y1 t1 y2 t2 =>
    ...
    | Some t0', _ =>
      Some (tm_case t0' y1 t1 y2 t2)
    | None, <{ inl T v }> =>
      assert (valuef v) (Some <{ [y1:=v]t1 }>)
    | None, <{ inr T v }> =>
      assert (valuef v) (Some <{ [y2:=v]t2 }>)
    | None, _ =>
      None
    end
  (* lists (the [tm_lcase] is given for free) *)
  ...
  | <{ t1.fst }> =>
      ...
      | Some t1', _ =>
          Some <{ t1'.fst }>
      | None, <{ (v1, v2) }> =>
          assert (andb (valuef v1) (valuef v2))
            (Some v1)
      | None, _ =>
          None
      end
...
Proof.
  intros t H.
  induction t; simpl in H; try discriminate; eauto.
  all: try (apply andb_true_iff in H;
            destruct H;
            eauto).
  (* - (* cons *)
    ...
    destruct H as [H1 H2].
    (* can just do eauto. *)
    constructor; try apply IHt1; try apply IHt2; assumption.
  - (* pair *)
    apply andb_true_iff in H.
    destruct H as [H1 H2].
    eauto. *)
...
Proof.
  ...
  - (* fst *)
    unfold assert in H.
    destruct (valuef t0_1) eqn:Hv1; simpl in H; try discriminate.
    destruct (valuef t0_2) eqn:Hv2; simpl in H; try discriminate.
    inversion H; subst.
    eauto using sound_valuef.
  - (* snd *)
    unfold assert in H.
    destruct (valuef t0_1) eqn:Hv1; simpl in H; try discriminate.
    destruct (valuef t0_2) eqn:Hv2; simpl in H; try discriminate.
...
Proof.
  ...
  induction H; simpl;
    ...
    try (
      match goal with
      | Hv : value ?v |- context [stepf ?v] =>
          rewrite (value_stepf_nf v Hv)
      end
    );
    try (
      match goal with
      | Hv : value ?v |- context [valuef ?v] =>
          rewrite (complete_valuef v Hv)
      end
    );
    try reflexivity.
