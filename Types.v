...
Proof.
  ...
  - inversion Hb; subst;
    destruct Hs as [t' step]; inversion step.
  - induction Hn.
    + inversion Hs. inversion H.
    + destruct Hs as [t' step].
...
Proof with eauto.
  ...
  inversion H2; subst; try reflexivity; try solve_by_invert.
  - apply IHstep in H5.
    rewrite H5.
    reflexivity.
  - apply IHstep in H0.
    rewrite H0.
    reflexivity.
  - assert (Hv : value <{ succ v }>).
    { right. constructor. exact H. }
    pose proof (value_is_nf <{ succ v }> Hv) as Hnf.
Admitted.
(** [] *)
...
Proof.
  ...
  - (* T_Succ *)
    destruct IHHT as [Hv | Hstep].
    + left. right.
      apply (nat_canonical t1 HT) in Hv.
      apply nv_succ.
      exact Hv.
    + right.
...
Proof.
  ...
  induction HT;
    ...
    - (* T_Pred *) inversion HE; subst; try assumption.
      + inversion HT. assumption.
      + apply IHHT in H0. apply T_Pred. assumption.
    - (* T_Zero *) inversion HE; subst.
      + apply T_True.
      + apply T_False.
...
Proof.
  right.
  intros H.
  specialize (H <{ if true then 0 else false }> <{ 0 }> Nat).
  assert (<{ |-- <{ if true then 0 else false }> \in Nat }>).
  {
    apply H.
    split.
    - apply ST_IfTrue.
