...
Proof.
  ...
  - unfold normal_form in P12, P22.
    ...
    + exfalso.
      ...
      exists y.
      apply H.
  - unfold normal_form in P12, IHP11, P22.
    inversion P21; subst.
    + exfalso.
      apply P22.
      exists y.
...
Proof.
  ...
  - eapply multi_trans with (y := P (C n1) t2).
    ...
    + eapply multi_trans with (y := P (C n1) (C n2)).
      * apply multistep_congr_2. apply v_C. exact IHeval2.
      * apply multi_R. apply ST_PCC.
Qed.
(** [] *)
...
Proof.
  ...
  - apply E_P. apply E_C. apply E_C.
  - apply E_P. apply IHHs in H1. exact H1. exact H3.
  - apply E_P. exact H2. apply IHHs in H4. exact H4.
Qed.
(** [] *)
...
Proof.
  ...
  induction Hmulti.
  - assert (value x) as V.
    { apply nf_is_value. exact Hnf. }
    inversion V; subst.
    exists n.
    split.
    + reflexivity.
    + apply E_C.
  - destruct IHHmulti as [n [Heq Heval]].
    + exact Hnf.
    + exists n.
      split.
      * exact Heq.
...
Proof.
  ...
  - inversion H5; subst; solve_by_invert.
...
Proof.
  right.
  intros H.
  specialize (H (P tru fls)).
  destruct H as [Hval | [t' Hstep]].
  - inversion Hval.
  - inversion Hstep; subst; try solve_by_invert.
Qed.
(** [] *)
...
Definition compiler_is_correct_statement : Prop
  := forall (st : state) (e : aexp),
    stack_multistep st (s_compile e, nil) (nil, [ aeval st e ]).

Lemma compiler_is_correct_aux :
  forall (st : state) (e : aexp) (stk : stack) (p : prog),
    stack_multistep st (s_compile e ++ p, stk) (p, aeval st e :: stk).
Proof.
  intros st e.
  induction e; intros stk p; simpl.
  - eapply multi_step.
    + apply SS_Push.
    + apply multi_refl.
  - eapply multi_step.
